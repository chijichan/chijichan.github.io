...
一个纯静态 **AI 聊天**，基于 **Supabase** 作为后端，可部署到 **GitHub Pages**。

## 项目结构

```
chijichat-chatroom/
├── index.html       # AI 聊天前端（所有代码都在一个文件）
└── .gitignore
```
...
使用 **Supabase** 作为后端服务：
- **数据库** — Supabase PostgreSQL 存储对话和 AI 配置
- **实时订阅** — Supabase Realtime 同步消息
- **AI 接口** — 浏览器直接调用兼容 OpenAI 格式的 API
...
| Supabase | 数据库 + 实时 + 配置存储 |
| OpenAI API | AI 对话（兼容格式） |
| GitHub Pages | 静态托管 |
4. 等待项目创建完成（约 1-2 分钟）
